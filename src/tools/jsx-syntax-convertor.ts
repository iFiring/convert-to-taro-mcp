...
export async function jsxSyntaxConvertor(options: ScanFilesOptions = {}): Promise<SyntaxConvertorResult> {
  try {
    ...
    try {
      ...
      if (scannedData.jsx && scannedData.jsx.length > 0) {
        waitingJsxFiles = scannedData.jsx.map((file: string) => `- ${file}`).join('\n');
      } else {
