...
import type { SyntaxConvertorResult, ScanFilesOptions } from '../types.js';
import { getPromptsPath } from '../utils.js';

export async function scssSyntaxConvertor(options: ScanFilesOptions = {}): Promise<SyntaxConvertorResult> {
  try {
    ...
    try {
      ...
      if (scannedData.scss && scannedData.scss.length > 0) {
        waitingScssFiles = scannedData.scss.map((file: string) => `- ${file}`).join('\n');
      } else {
